...
namespace FormattedExcelExport.Configuaration {
	public sealed class TableConfigurationBuilder<TModel> {
	    ...
	    private readonly Func<dynamic, dynamic> _getter = model => {
	        ...
	        if (model is bool?) value = model != null ? (model == true ? "ƒа" : "Ќет") : string.Empty;
	        return value;
	    };
        ...
        //todo подумать как убрать дублирование (сейчас нужно дл€ контрол€ типов, т.к. дальше примен€етс€ dynamic)
